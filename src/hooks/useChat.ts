...
export function useChat({ initialCurrentUser }: UseChatProps) {
    ...
    const handleClearChat = useCallback(async () => {
        if (!activeChatId) return;
        try { await api.clearChatHistory(activeChatId); toast({ title: "Chat Cleared" }); } 
        catch (error: any) { toast({ variant: "destructive", title: 'Clear Failed', description: error.message }); }
