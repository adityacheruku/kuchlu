...
export default function AuthPage() {
  ...
  const handleVerifyOtp = async (e: FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      const response = await api.verifyOtp(`+91${regPhone}`, regOtp);
  ...
  const handleCompleteRegistration = useCallback(async (e: FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    try {
      const regData: CompleteRegistrationRequest = { registration_token: registrationToken, display_name: regDisplayName, password: regPassword, email: regOptionalEmail || undefined };
      await completeRegistration(regData);
    } catch (error: any) {
      console.error("Registration error:", error.message);
    } finally {
      setIsSubmitting(false);
    }
  }, [completeRegistration, registrationToken, regDisplayName, regPassword, regOptionalEmail]);
